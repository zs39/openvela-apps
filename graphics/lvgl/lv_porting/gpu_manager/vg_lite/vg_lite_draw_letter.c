    ...
    uint32_t letter)
{
    ...
    if(!g_ret) {
        ...
        if(letter >= 0x20 && letter != 0xf8ff &&  /*LV_SYMBOL_DUMMY*/
           letter != 0x200c) { /*ZERO WIDTH NON-JOINER*/
            LV_GPU_LOG_INFO("lv_draw_letter: glyph dsc. not found for U+%" LV_PRIX32, letter);
    ...
    if(draw_letter_normal(draw_ctx, dsc, &gpos, &g, letter) == LV_RES_OK) {
        gpu->draw_ok = true;
    }
}
...
#ifndef LV_GPU_VG_LITE_LVGL_BLEND_SUPPORT
    /* Non-lvgl blend modes require premultiplication */
    if(opa < LV_OPA_MAX) {
                                   ...
                                   const lv_point_t * pos_p, lv_font_glyph_dsc_t * g, uint32_t letter)
{
    ...
    if(!matrix_inverse(&result, &matrix)) {
        LV_GPU_LOG_ERROR("no inverse matrix");
        return LV_RES_INV;
    }
    ...
    LV_ASSERT(vg_lite_custom_buffer_init(
                  &dest_vg_buf,
                  draw_ctx->buf,
    ...
    VG_LITE_CHECK_ERROR(vg_lite_draw(
                            ...
                            &matrix, blend, vg_color_make(dsc->color, dsc->opa)));
...
static lv_freetype_outline_t * vg_lite_outline_create(lv_freetype_event_dsc_t * event_dsc)
{
    ...
    if(!outline) {
        LV_GPU_LOG_WARN("outline alloc failed");
        return NULL;
    }
    lv_memset_00(outline, sizeof(vg_lite_outline_t));
    ...
    if(!path_data) {
        LV_GPU_LOG_WARN("path_data alloc failed");
        lv_mem_free(outline);
        return NULL;
    }
...
static void vg_lite_outline_push(lv_freetype_event_dsc_t * event_dsc)
{
    ...
    if(offset >= path_data_len) {
        LV_GPU_LOG_ERROR("offset = %d, path_data_len = %d, overflow!",
