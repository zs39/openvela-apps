...
typedef struct {
  uint8_t  used;
  uint32_t hash;
  uint32_t mem[VLC_GRADBUFFER_WIDTH];
} lv_gpu_grad_img_map;
...
LV_ATTRIBUTE_FAST_MEM static void grad_mem_map_clear(void){
  size_t size = sizeof(grad_mem_map) / sizeof(grad_mem_map[0]);

  for (int i = 0; i < size; i++) {
    grad_mem_map[i].used = 0;
  }
}
    ...
    const lv_gpu_buffer_t* gpu_buf)
{
  if (!gpu_buf || !gpu_buf->buf || !curve || !curve->op
    ...
    LV_LOG_ERROR("Invalid argument");
    return LV_RES_INV;
  }
  if (curve->fill->type == CURVE_FILL_LINEAR_GRADIENT && !curve->fill->grad) {
    LV_LOG_ERROR("Invalid gradient argument");
    return LV_RES_INV;
  }
    ...
    lv_gpu_curve_fill_t* gpu_fill, const lv_gpu_buffer_t* gpu_buf)
{
  if (!path || !length || !gpu_fill) {
    LV_LOG_ERROR("invalid arguments");
    return LV_RES_INV;
  }
  ...
  *p_lastop = VLC_OP_END;
  ...
  if (type == CURVE_FILL_COLOR) {
    ...
    if (!ggrad) {
      *p_lastop = original_op;
      LV_LOG_ERROR("no grad dsc");
      return LV_RES_INV;
    }

    vg_lite_linear_gradient_t grad;
    lv_grad_dsc_t* lv_grad = ggrad->grad_dsc;
    if (LV_RES_OK != gpu_grad_img_init(&grad, lv_grad, opa)) {
      LV_LOG_ERROR("grad img init failed");
      return LV_RES_INV;
    }
...
/****************************************************************************
 * Name: gpu_grad_img_init
 *
 * Description:
 *   Init the grad img buffer
 *
 * @param vg_grad pointer of vglite gradient
 * @param lv_grad pointer of lvgl gradient
 ...
 *
 ****************************************************************************/
lv_res_t gpu_grad_img_init(void* vg_grad, const lv_grad_dsc_t* lv_grad, lv_opa_t opa)
{
  uint32_t grad_hash = calc_grad_hash(lv_grad);
  int_fast16_t k;
  int_fast16_t unused_idx = -1;
  int_fast16_t hash_idx = -1;
  int_fast16_t mem_idx;
  size_t map_size = sizeof(grad_mem_map) / sizeof(grad_mem_map[0]);
  vg_lite_linear_gradient_t* vg_lite_grad = (vg_lite_linear_gradient_t*)vg_grad;

  for (k = 0; k < map_size; k++) {
    if (!grad_mem_map[k].used) {
      unused_idx = k;
    } else if(grad_hash == grad_mem_map[k].hash) {
      hash_idx = k;
      break;
    }
  }
  ...
  if(init_vg_buf(&(vg_lite_grad->image), VLC_GRADBUFFER_WIDTH, 1,
        VLC_GRADBUFFER_WIDTH * sizeof(uint32_t), grad_mem_map[mem_idx].mem,
        VG_LITE_BGRA8888, false) != LV_RES_OK) {
    return LV_RES_INV;
  }

  vg_lite_grad->count = lv_grad->stops_count;
  for (int_fast16_t i = 0; i < vg_lite_grad->count; i++) {
    lv_color_t color = lv_grad->stops[i].color;
    if (opa < LV_OPA_MAX) {
      color = lv_color_mix(color, lv_color_black(), opa);
      LV_COLOR_SET_A(color, opa);
    }
    vg_lite_grad->colors[i] = lv_color_to32(color);
    vg_lite_grad->stops[i] = lv_grad->stops[i].frac;
  }

  if (hash_idx == -1 && vg_lite_update_grad(vg_lite_grad) != VG_LITE_SUCCESS) {
    return LV_RES_INV;
  }

  grad_mem_map[mem_idx].used = 1;
    ...
    lv_img_header_t* header, const char* ext)
{
  ...
  if (cf == LV_IMG_CF_TRUE_COLOR || cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
    ...
    LV_LOG_ERROR("Filter does not support this color format!");
    return LV_RES_INV;
  }
  return LV_RES_OK;
}
...
const char* generate_filtered_image(const char* src)
{
  ...
  if (res != LV_FS_RES_OK || bytes_done != 4) {
    LV_LOG_ERROR("read header failed");
    goto Error_file;
  }
  ...
  if (res != LV_FS_RES_OK || bytes_done != 4) {
    LV_LOG_ERROR("write header failed");
    goto Error_file;
  }
  if (!dst_buf) {
    dst_buf = data;
  }
  if (pre_zoom_gaussian_filter(dst_buf, data, &header, ext) != LV_RES_OK) {
    if (dst_buf == data) {
      dst_buf = NULL;
    }
    GPU_WARN("Filter failed");
    goto Error_file;
  }
  ...
  if (res != LV_FS_RES_OK || bytes_done != dst_size) {
    LV_LOG_ERROR("write data failed");
    goto Error_file;
  }
  lv_fs_close(&file);
  free(data);
